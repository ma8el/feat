...
### ADR-061 — A project is configured by answering questions, and the answers are checked before there is a file
...
### ADR-062 — One flow, two askers: the wizard's questions are a package, and the dashboard asks them itself
...
- `feat project init` keeps its conversation and owns its presentation: the
  ...
  diagnose and register at the end. ADR-061's reasons stand — it runs before
...
### ADR-063 — Diagnosis is read on the dashboard, and it says which process it is true of

Status: accepted  
Recorded: slice 13, from the hole ADR-062 left
