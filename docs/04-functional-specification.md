...
- bind mounts naming a path inside a repository that Git does not track, because a task works in a worktree and a worktree holds only what Git tracks. This reports and does not refuse: a path a build step creates is a legitimate absence. An entry Feat could not read, because it interpolates, MUST be reported as unchecked rather than passed over (ADR-081). Both mount checks MUST be silent about a repository a task mounts no worktree of — one omitted from tasks, or one kept stable and read-only, which a task reads from the ordinary checkout — because every claim they make is about a worktree (ADR-098);
- bind mounts writing a **file** into where Feat mounts a task's worktree, on a path the worktree would not hold, because the container runtime has to create that mount point inside the worktree and some runtimes refuse to create a file outside the container. Where Feat has established that this runtime refuses, it MUST report an error and fail the run, unlike the case above: the failure happens at container creation, so no build step and no `postCreateCommand` runs early enough to be the legitimate absence that one allows for. Where it has not, the finding MUST still be reported, as a warning naming both measured outcomes, rather than claimed either way — a runtime that creates the file starts the container, so an error there would refuse a project that works. A mount whose source is a directory, or absent, is not reported on any runtime, because a directory mount point is created and the container starts (ADR-098);
