...
- bind mounts writing a **file** into where Feat mounts a task's worktree, on a path the worktree would not hold, because the container runtime has to create that mount point inside the worktree and refuses to create a file outside the container. This MUST report as an error and fail the run, unlike the case above: the failure happens at container creation, so no build step and no `postCreateCommand` runs early enough to be the legitimate absence that one allows for. A mount whose source is a directory, or absent, is not reported, because the runtime creates a directory mount point and the container starts (ADR-098);
