...
// The files Linux reports the machine's state in. Nothing is executed here, because
// the kernel publishes both and reading a file cannot fail in the ways a process
// can.
const (
...
//
// Linux publishes processor times in /proc/stat, and a true utilisation percentage
// could be computed from two readings of them. Feat reports load here anyway, because
// macOS has no equivalent Go can reach without cgo, and one dashboard heading must
// not mean a percentage on one platform and a load average on the other (ADR-035).
...
// readMemory reports the machine's total and available memory. MemAvailable is the
// kernel's own estimate of what a new process could get without swapping, which is a
// better answer than MemFree.
...
//
// Linux types the block size as a signed integer, and a negative one would wrap into
// an enormous block and report a filesystem larger than the machine. It cannot happen
// on a working kernel, so a figure this build cannot trust is reported as unmeasured,
// as every other unavailable figure is (ADR-035).
