...
// HostRunner runs the container tool on the trusted host. The agent's container
// receives no socket and no CLI, so the runtime the user starts from Feat is a
// host action even though the services it starts are the ones the agent's code
// will be tested against (docs/05-security-model.md, Docker boundary).
...
// defaultTimeout bounds one Docker command. It is generous because bringing an
// application up can pull images or run builds, and it exists because a Compose
// that never answers should produce a diagnosis rather than a hanging request.
