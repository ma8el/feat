...
// treated as too old: refusing to start an application because a tool printed
// something unfamiliar would make every Docker release an outage, and the
// failure it guards against announces itself clearly when it happens.
type Version struct {
	Major, Minor int
	Text         string
	Parsed       bool
