...
// osascriptProgram is macOS's own script runner, named by absolute path because a
...
//
// The text is passed as arguments and read out of argv rather than interpolated into
// the script, for the reason Git, tmux, and Docker Compose take argument vectors. A
// task title is text the user typed, and one containing a quotation mark would
// otherwise end the string it was pasted into and leave the rest to be executed.
var script = []string{
	"on run argv",
...
//
// It answers whether Feat can hand one over, which is not whether the user will see
// it. macOS decides that per application, drops an unauthorised notification without
// saying so, and osascript exits 0 either way. Feat therefore reports that it
// delivered a notification and never that one was seen.
