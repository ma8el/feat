...
// service answers the local API from persistent state. It is the only component
// that holds the store, because the daemon is the sole reader and writer of
// persistent state (ADR-008) and confining it to one type makes that checkable.
type service struct {
	store store.Store
	bus   *Bus
	...
	// user-supplied commands, and a test may want one to answer while the other
	// cannot.
	tracker tracker.Runner

	// forges open merge requests, one adapter per forge a repository can declare.
	// Every credentialed provider call goes through one of them on this host, with
	// the authentication the user already has, because the agent environment
	// receives no provider token (ADR-070).
	...
	// (ADR-034), so a test can refuse to start an application while the agent's own
	...
	// hostAgent reports that the daemon was started with the opt-in that launches
	// an agent on this host even where the project configures a container. It is
	// read from the daemon's own environment and never from a request, because a
	// caller that could move the agent would be granting itself a capability
	// (ADR-032).
	hostAgent bool
	build     Build
	endpoint  Endpoint
	...
	// undeliverable is why this build delivers no desktop notification, asked once
	// at startup and empty when it can. A notification this platform cannot show is
	// dropped saying so, rather than logged as a failed delivery.
	...
	// never nil. They hold dispositions of the machine and the person at it, which
	// change about as often as the machine does, so changing one takes a restart
	// and `feat settings show` says so (ADR-079).
	...
	// runtime action. Only a test sets it, so a Compose command that outlasts its
	// budget can be observed without a test that runs as long as the budget.
	...
	// ticketOverride replaces api.TicketTimeout as the budget for one listing of a
	// project's tickets. Only a test sets it, for the reason above.
	...
	locks *taskLocks
	// portMu serialises host port allocation across every task, because choosing a
	// free port means reading what every other task holds and two tasks created at
	// once would both read the same port as free. It is held across the record as
	// well as the choice, since the next task cannot see a choice nothing saved.
	portMu sync.Mutex
	// reserving runs between a task's ports being chosen and being recorded, which
	// is the window portMu closes. Only a test sets it: every Docker command a fake
	// can hold up runs after both halves, so without this seam a test cannot enter
	// the window at all.
	reserving func(domain.TaskID)
	// handovers records the terminals a client has been sent to attach to, so a
	// rendering does not pin a window out from under a client on its way to it. It
	// is separate from locks, because a frame must not wait behind a launch or a
	// completion gate holding a task's lock for as long as its work takes.
	handovers *handovers
...
// Health reports what the daemon knows about itself. Unreadable state produces a
// degraded report rather than a failed request, because a client asking whether
// the daemon is alive learns more from "running, but the state directory cannot
// be listed" than from a 500.
...
//
// The daemon does the reading as well as the writing. The request names a
// project, and the file is the one at the documented location under the
// configuration directory this daemon resolved, so a client cannot point the
// daemon at a file of its choosing.
//
// Registration is idempotent: re-registering picks up an edited configuration and
// keeps the original registration time. Tasks already running are unaffected,
// because their configuration was resolved into a launch snapshot
...
// configOptions returns the settings configuration is resolved against. The
// daemon's own environment supplies them, so a resolved "~" is the user who owns
// the daemon rather than whoever asked.
func (s *service) configOptions() config.Options {
	return config.Options{Env: s.env, StateDir: s.layout.State}
}

// translateConfig maps a configuration failure onto the API vocabulary. A missing
// file and an invalid one have different remedies: one is a file to write, and
// the other is a file to fix.
...
// Task returns one task addressed by task identifier alone. Storage addresses a
// task by project and task together (ADR-026), and the command surface addresses
// it by itself, because the user types `feat attach <task>`. Resolving one to the
// other is the daemon's job (ADR-027).
...
// ResolveTask turns what a user typed into the task it names. ADR-027 addresses a
// task by task identifier and leaves the daemon to resolve the owning project;
// this decides which identifier a user has to be able to see. Lists show the
// eight-character key, so the key names a task, and so does any prefix of an
// identifier.
//
// Both failures name where a valid value comes from, because explaining an
// identifier's format to somebody who cannot see one helps nobody.
...
// Publish records a state change on the event stream. The delivery path's order
