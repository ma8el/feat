...
// adopted: a stopped container is reported as stopped (FR-STATE-004), and every
// action a report suggests is one the user takes. A step that fails does not end
// the pass either, because its failure is a problem on the report and the
// remaining steps still run (ADR-037).
func (s *service) Reconcile(ctx context.Context) (api.Reconciliation, error) {
	report := &reconcile.Report{StartedAt: s.now()}

	// From what claiming the state directory read rather than from the record on
	// disk: claiming has replaced that with this run's own, and re-reading it would
	// report this daemon's start as the previous daemon's end.
	...
	// Asked once, before either pass that says anything about it. The terminal pass
	// has to say whether anything of the task is lost and the environment pass names
	// what a launch left, and a report whose two halves disagree about one container
...
// previousDaemon returns what the last run left behind. It prefers what claiming
// the state directory read, because by the time a pass runs the record on disk
// describes this run. A nil record and a nil error mean no daemon has owned this
// directory before.
...
// reconcilableTasks lists every task of every project. A project whose tasks
// cannot be listed is a problem rather than the end of the pass, because a user
// with one unreadable project should not lose the recovery report for the rest.
...
// reconcileTerminals compares stored sessions with tagged tmux objects. A stale
// stored identifier is repaired from live metadata, a missing terminal is marked
// stopped without anything being restarted, a terminal whose metadata claims
// another project is reported rather than adopted, and a damaged terminal is a
// finding rather than the end of discovery.
...
func (s *service) reconcileTerminal(
	...
	// Re-read under the lock. Nothing else writes during startup, but the on-demand
	// pass runs beside every other writer, and a load-change-save cycle starting
	// from a copy taken outside the lock is the defect ADR-036 evidence 9 records.
	...
	// What the terminal establishes and no more. A live pane under a session already
	// recorded in an alive state establishes nothing the record does not hold, and
	// promoting it would take a session the provider reported idle back to running
	// for the rest of its life (ADR-096).
	...
	if adopted || target != current.Session.Tmux || from != current.Session.Process {
		...
		if adopted {
			// There was no state to move from: the session this names is the one the
			// line above created, and a from repeating the to would read as a
...
//
// It is the one place that writes this observation, because three callers make it
// and they must not describe it differently: a reconciliation pass that found
// nothing where the record said, a resume that asked tmux the same question, and
// a resume that found the terminal but not the container. The reason is the
// caller's; the state, the event, and their order are not.
//
// Nothing is restarted here. What the caller does next is the caller's, and only
// one of them does anything at all.
...
//
// Recovery is offered and never performed, because a report that restarted what
// it found would be deciding for the user. ADR-032 deferred the decision here so
// it could be made once for every resource class.
//
// An action names something a user can do. Nothing launches a task that is past
// draft, and a task with no recorded session has never held an agent
// conversation, so cleaning it up and preparing another loses nothing but the
// brief.
//
// The command is named beside the key, because a recovery a user can read about
// but not reach from where they are reading is not one the product offers
// (ADR-057).
...
// removed by hand stays removed, and the record that named it explains the gap.
...
// asking what Feat manages here and what is in use right now. A project with no
// task still has a worktree root and its own directory in it, and a scan that
// learned both from tasks would report the directory of a project between tasks
// as one nobody claims.
//
// A project whose configuration cannot be read is a problem rather than the end
// of the pass, reported once for the project rather than once per task, because
...
func (s *service) worktreeDirectories(
	...
	for _, project := range registered {
		...
		// The first project to name a root keeps it. Several projects share one under
		// the default configuration, and the identifier here only says whose report a
		// failure to list that directory belongs in.
...
// claims.
//
// They are reported and never removed. A directory Feat created and lost track of
// and a directory the user put there look identical from here, and only one of
// them is safe to delete.
//
// The scan descends where Feat's own paths lead and reports what is beside them.
// A root of `…/worktrees/{project_id}/{task_id}` puts the interesting directories
// two levels down, so a scan of one level would report a live project's directory
// as an orphan and miss the stale task directory inside it. A task's recorded
// worktree and a registered project's own directory lead the walk, so its depth
// comes from the templates rather than from a number somebody chose.
...
func (s *service) scanForOrphans(
	...
	if err != nil {
		if os.IsNotExist(err) {
			// No worktree root yet is not a problem: the first task creates it.
			return
		}
	...
	for _, entry := range entries {
		...
		case owned:
			// A registered project's own directory, which is never reported: Feat
			// generated it and the project's next task is created inside it. What is
			// inside it is still looked at, and reported as that project's.
		...
		case holdsAClaimedWorktree(path, claimed) || holdsAProjectDirectory(path, projects):
			// On the way to one, so it belongs to a task or a project even though
			// nothing names it. Keep looking inside it.
...
// task claims.
//
// An empty one is named as empty. Most of what is found here is the residue of
// cleanups the user confirmed themselves, and "look at it and remove it if it is
// stale" would send somebody to the end of a path to find nothing there. Cleanup
// takes those directories with the worktree now, and the ones already on a
// machine stay until somebody removes them.
//
// The path is not repeated into the action, because the finding already names it
// and the dashboard truncates a line rather than wrapping it. Empty or not,
// nothing is removed from here: reconciliation reports and the user acts
// (ADR-037).
...
// registered project's worktrees are created in. A root of
// `…/worktrees/{project_id}/{task_id}` puts it one level down, a template may put
// it deeper, and every directory on the way is Feat's own.
...
// reconcileEnvironments observes each task's agent container, and nothing else. A
// stopped devcontainer stays stopped, because bringing one up is what a resume or
// a launch does and both are things a user asked for (FR-STATE-004).
...
// reportUnrecordedEnvironment reports the containers of a task whose record names
// none.
//
// It is the half of this pass F2-15 found missing. A launch that fails after its
// container exists records no environment, and a pass that asked only about the
// tasks with one left those containers and networks in no report and in no
// cleanup plan.
//
// Nothing is adopted and nothing is removed. The finding names the task the
// resources were created for, because the Compose project name is that task's
// own, and the action is the cleanup that resolves them.
//
// It describes one moment, as every finding here does. A launch in flight has its
// container before it has its session, so a pass during those seconds reports
// what is true then and the next one reports the same container as present.
//
// The moment is the one unrecordedEnvironments read rather than a second look of
// this function's own, because two Docker queries a few milliseconds apart are
// two moments a report could describe differently.
...
// have on the machine. It asks once per task per pass and Reconcile shares the
// answer. It is a derivation and not a scan, because the Compose project name
// comes from the task's own two identifiers (ADR-059). Nothing is adopted,
// started, or removed here or in either pass that reads it.
...
//
// "its agent never ran, so nothing it did is lost" is the reassurance the finding
// exists to give: a task confirmed and interrupted before its terminal existed
// can be cleaned up without a thought. It is untrue of a launch that failed after
// its container existed, which is also a task with no session, so the answer this
// pass already has decides the sentence.
...
//
// It also applies the invariant tying the two halves of a session together: an
// agent process cannot be alive while the environment it runs in is not running.
// The terminal pass cannot see a devcontainer's death, because the pane's process
// is on the host side of the container and outlives it, so one pass would
// otherwise carry a finding saying to resume and a resume refusing because an
// agent was already running (ADR-057).
//
...
// observes the container. An alive process against a container that is not
// running is a death nobody asked for.
//
// None of it is the automatic restart FR-STATE-004 forbids. Nothing is started or
// stopped here, and what changes is what the record says about a process that has
// already ended.
...
// reconcileRuntimes observes each task's application services. It never starts
// anything (FR-STATE-004), and it reuses the runtime poller's observation rather
// than adding a second one, so a task's runtime state has one definition.
...
// reconcileControl reports the control messages a restart found waiting. The
// poller that runs immediately after applies them, which is what makes a turn
// that ended while Feat was stopped reach the dashboard. What belongs here is
// what the poller cannot report: a document that will never parse, and a
// workspace that has gone missing under a task that still names it.
...
// verifying with no gate running is claiming that checks are running when nothing
// is. Reconciliation reports it rather than only doing it (ADR-036 evidence 2).
//
// "With no gate running" is the part ADR-036 left implicit. Reconcile is an API
// request as much as a startup step — the dashboard makes it on a key press, on a
// resume, on a stop, and after every cleanup action — so the gates this daemon is
// running are asked about rather than assumed absent. A pass that assumed them
// absent moved a task back to review_requested five seconds after its gate
// started, and the results then landed in a task that was no longer verifying
// (ADR-096).
...
// lock. What it records is that the checks are not running, which is what the
// caller established, rather than the restart that usually caused it: a gate this
// daemon released without finishing reaches the same state without a restart
// (ADR-096).
...
// startupReconcile runs the pass before the daemon serves anything, and logs what
// it found. A failed pass must not stop the daemon: a user whose recovery report
// could not be produced still needs the dashboard that would show it, and the
// last recorded state is the best answer Feat has.
