...
// runtimeFixture is the preparation fixture with application services added. The
// services mount the repositories at the same container paths the devcontainer
// uses, which is what a project whose Compose files define both looks like, and
// what ADR-034's post-start inspection checks rather than assumes.
...
// contributing adds the repositories' parts of the application to a fixture that
// has none: the Compose files api brings, where each repository's code is
// expected inside the services that run it, and the service Feat manages.
//
// Both repositories contribute and only one brings a file. store's code runs in
// api's service, as a library the application depends on, which is what a
// per-repository container path is for: two repositories' worktrees reach one
// service at two paths of their own.
//
// It is spliced rather than appended because a repository's contribution lives on
// the repository, so what an application is made of is answered where its code is
// (ADR-065).
...
// TestStartingOneTaskDoesNotAffectAnother is the first acceptance criterion. It
// is checked at the adapter rather than at the outcome, because every Compose
// command carries a project name and one carrying this task's name cannot reach
// another task's services whatever the container runtime does with it.
func TestStartingOneTaskDoesNotAffectAnother(t *testing.T) {
	...
	for _, vector := range arranged.runtimes.Vectors() {
		...
		// that act on none — asking Docker its version, and inspecting a container by
		// identifier — are scoped by something else.
	...
	// And the other task's record is exactly as it was, because nothing asked about
	// it and so nothing observed it.
...
// TestADestroyPlanReachesNothingOutsideTheTasksOwnProject is the third acceptance
// criterion.
//
// The criterion was written about external database resources, and no rule
// excludes them: a destroy addresses the task's own Compose project and names
// nothing, so anything Feat does not own is beyond its reach by construction, and
// ADR-048 removed the declaration that used to restate it.
//
// The volumes are checked here because they are the other half of what a destroy
// must leave alone, and the half inside the project.
...
// TestTheRuntimeKeepsRunningThroughReview is the fourth acceptance criterion. A
// task reaching review is a statement about the work rather than about the
// services the user is testing it with, so nothing in the workflow may stop them.
// It checks that no Compose command ran rather than that the recorded state is
// unchanged.
func TestTheRuntimeKeepsRunningThroughReview(t *testing.T) {
	...
	// The task goes to work and then asks for review, which is the whole path past
	// the point where something might have tidied up after it.
...
// acceptance criterion. A task's services are the user's to keep or to end, and
// no workflow transition touches them, so the environment somebody is testing in
// outlives the state the work is in. `ready_for_review` is the furthest a task
// gets without cleanup (ADR-086).
...
// application services. The check is made from inside the operation that creates
// them: when Compose is asked to bring the services up, the snapshot on disk must
// already name the Compose project about to exist, or an interruption leaves
...
// combined Compose file anywhere. Each entry carries the repository's own checkout
// as its project directory, so its build contexts and relative mounts resolve
// against it. A single file list would resolve every relative path against one
// directory, and build the second repository's services from the first (ADR-065
// evidence 2).
func TestTheApplicationIsComposedOfItsRepositories(t *testing.T) {
	...
	// And it is Feat's, under the runtime root rather than in a repository the user
	// maintains by hand.
	root := arranged.layout.RuntimeRoot()
...
// TestAServiceHoldsTheCodeItRuns covers which worktrees reach which service. A
// repository's runtime container path is where its own services expect its
// source, so its worktree goes to those services and no others. Mounting every
// worktree into every service would turn two repositories expecting their source
// at the same path into a collision.
func TestAServiceHoldsTheCodeItRuns(t *testing.T) {
	// A second managed service, brought by the api repository and running only its
	// code, so the store repository's worktree must not reach it.
...
func TestTheTasksOwnCodeIsWhatTheServicesRun(t *testing.T) {
	...
	// The read-only repository stays read-only in the application too: a task that
	// may not write the code may not write it through a service either.
...
// found. A task binding records a container path only for devcontainer execution,
// because that field says where the agent's container mounts the worktree and a
// host-native agent has none. An application runtime has containers whatever the
// agent does, so reading the mount from the binding gave a host-execution project
// an override with no volumes: its services ran the user's ordinary checkout,
// every record Feat kept was correct, and only a note said otherwise.
func TestAHostExecutionProjectStillMountsItsWorktrees(t *testing.T) {
	...
	// Host execution sets these itself, and configuration refuses them. The agent's
	// own container paths go with them, because there is no agent container to mount
	// anything in, which is why reading the application's mounts from that field
	// left this project mounting nothing.
...
// over a refusal. The application runtime is inside the trusted host zone, so a
// checkout mounted into it is a correctness problem rather than a boundary
// breach: the user's change has no effect and nothing says why. Feat says why and
// leaves the services running.
...
// lifecycle. Nothing is undone, because a service that started may already have
// written to a volume or a shared database and tidying up after a failed start is
// a destructive act nobody asked for (ADR-029, ADR-033). What must survive is the
// record naming what may exist.
...
// TestADestroyedRuntimePicksUpAFixedConfiguration is the other half of that rule.
// Once nothing exists, re-resolving can orphan nothing, and a user who fixed
// their configuration after destroying everything gets the fixed one.
...
// dashboard. The dashboard re-reads state on every event, so a poll that
// published every time it looked would make every task with services a permanent
// source of reads.
...
// failure a user met as `context deadline exceeded` on a first start. The budget
// is the daemon's own (api.RuntimeTimeout), and what it produces has to be a
// diagnosis rather than a transport error: it names the action, the budget, and
// the command that says what is on the machine now. It is ErrInvalid for the
// reason every other Compose failure is, because that is what carries a message
// to the user.
func TestAStartThatOutlastsItsBudgetSaysWhatHappened(t *testing.T) {
	...
	// A Docker command that outlasts the whole action's budget. Which step holds
	// the clock when it runs out is not asserted here: the budget is one number for
	// the whole action and a loaded machine spends part of it before Docker is
	// reached, so pinning the step would pin how fast the machine is. What every
	// step owes the user is checked below, without a clock.
	...
	// Nothing was claimed about services nobody managed to look at. The record still
	// names the Compose project, which is what makes whatever Docker created
...
// failed for the same reason as one whose clock goes inside `up`, and the user is
// owed the same three things either way: the action, the budget, and the command
// that says what is on the machine now. Reporting them from only one step made
// the other arrive as `cannot manage its application services: context deadline
// exceeded`, and which a user met depended on how loaded their machine was.
//
// What differs is whether there is a half-finished Compose command to warn about.
...
// command is cut short. A caller that disconnected and a budget that ran out both
// leave a half-created Compose project, and only one of them is Feat's own
// patience. The daemon logs this one as well as reporting it, because the
// connection that would have carried the report is what has gone.
...
// unreachedFixture gives the store repository a service of its own and no runtime
// container path. It is a managed service nothing of the task reaches: no
// worktree is mounted into it, and the fixture's Compose file builds nothing from
// that repository. The project still loads, because the api repository says where
// its own source goes, which is the case a configuration rule cannot catch.
...
//
// A service that mounts no task worktree and builds from none runs whatever the
// project's own files give it, which is the user's ordinary checkout. That is
// otherwise silent: the containers start, the application serves, and every
// record Feat keeps stays correct (ADR-065 evidence 7). It is resolved from
// configuration when the runtime is, so a create says it before a container
// exists.
//
// The record is read at the first Compose command of the action, which makes the
// rule observable: the answer is on the task before Compose has been asked
// anything, so no path to it can be `docker compose config`, which would render
// the values of the project's environment files (ADR-034 evidence 5).
func TestAServiceThatRunsNoTaskCodeSaysSoBeforeAnythingStarts(t *testing.T) {
	...
	// And it is said in words, on the action a user asked for, rather than left in
	// a record they would have to go looking through.
...
// composition exists for. A service whose image copies its source in has no mount
// to replace, so the container path decides nothing and ADR-034's post-start
// inspection finds no mount to report. Only the build context decides what such a
// service runs (ADR-065 evidence 4).
func TestABuiltServiceIsPointedAtTheTaskWorktree(t *testing.T) {
	arranged := arrangeConfigured(t, runtimeFixture)
	// The project's own Compose file, in the shape a real one is written: a build
	// context of "." beside an interpolated build argument Feat never reads, and no
	// mount anywhere.
...
// task's code and will not show a change looks like. The agent that changed the
// code is confined to a devcontainer with no Docker and can rebuild nothing
// (ADR-065 evidence 9), so the user has to be told what makes the change appear.
func TestABakedServiceSaysItNeedsBuildingAgain(t *testing.T) {
	// The store repository brings a service of its own, built from its own checkout
	// and mounting nothing: the shape of a frontend whose image is a multi-stage
	// build ending in a web server. The api repository still says where its own
	// source goes, so configuration cannot fault the project.
	fixture := strings.Replace(runtimeFixture,
		"    runtime:\n      container_path: /srv/store\n      services:\n        - api\n",
...
// TestAServiceThatMountsAndBakesStillSaysItNeedsBuilding is the shape a real
// project turned out to have, and the rule it corrected. A service that mounts a
// repository and builds from it was reported as current, on the reasoning that
// the mount is what it reads. That holds for an application server reloading from
// its mounted source and not for a web server serving what its build produced,
// and a project running one of each served the task's work only because the
// second had been rebuilt (ADR-065 evidence 15).
...
// TestCleanupRemovesTheGeneratedRuntimeInput is ADR-037 evidence 16 at the second
// of the two roots it names. The application's generated Compose document has the
// same lifetime as the agent's: it is written per task under
// `<state>/runtime/<project-id>/<task-id>/`, it defines the Compose project, and
// nothing removed it. The directory is also the working directory of every
// Compose command Feat runs for the task, so it goes after the destroy.
func TestCleanupRemovesTheGeneratedRuntimeInput(t *testing.T) {
	...
	// destroy leaves and the one the plan is resolved against.
	arranged.runtimes.Answer("ps --all --format json", "")
