...
type briefModel struct {
	// task is whose brief the offset belongs to. The offset survives leaving the
	// tab and coming back, and is discarded when the selection moves: half way
	// down one task's brief is nowhere in particular in another's.
	task string
...
// openBrief shows the selected task's brief. Nothing is asked of the daemon:
// the brief travels on the task the rail is already drawing. The tab opens
// whatever it finds, including nothing at all, because a tab that declines to
// open is one the cycle cannot pass (ADR-041).
...
// wrappedBrief is the brief re-flowed to the width it will be drawn at. Nearly
// all of this body is text Feat did not write, and a brief carries tabs and
// carriage returns that are worth nothing to the wrap and everything to the
// terminal (ADR-054). Wrapping before the split also keeps the scroll honest:
...
// briefPanel is one task's brief under a heading that says whose it is and
// where it came from. A task with no brief and a task that is no longer listed
// each say so, because this tab is reached by cycling and a region that goes
// blank reads as a dashboard that broke.
func (m Model) briefPanel(width int) string {
	...
	// Where the brief came from is a fact about the document and not part of it,
	// so it goes beside the heading. The header drops an aside that does not fit
	// rather than truncating it, and half a path says nothing.
...
// briefHints names the one key this tab claims for itself.
