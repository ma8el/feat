...
// The setup skill, as Claude Code discovers it: a directory named for the skill
// under the skills directory, holding a SKILL.md (ADR-093).
//
// Beside the document sits the install record, which is the Feat build that
// wrote the file and a checksum of what was written. It lets a later install
...
// skillDocument is the canonical setup skill. It is embedded so the skill on
// disk is written by the build it matches: one binary carries the document, the
// schema, and the example the document sends its reader to (ADR-093).
//
//go:embed skill.md
var skillDocument []byte

// envClaudeConfigDir is Claude Code's own override for where its configuration
// lives, skills included. When it is set, a skill written to ~/.claude would be
// one Claude Code never reads.
...
// install recorded writing. The two causes, that the user edited the file or
// that something else wrote it, are what the record exists to separate from
// Feat's own earlier install, and neither is replaced unasked (ADR-093).
...
type skillRecord struct {
	...
	// algorithm so a future change of it reads as a difference rather than as a
	// silent mismatch.
	Checksum string `json:"checksum"`
}

// Permissions for the skill directory and its files. The document is not a
// secret, being the same text every build carries, and Claude Code has to be
// able to read it.
const (
...
// Skill returns the setup skill document, exactly as InstallSkill writes it. It
// is returned as a copy, so what the build embedded is what every caller
// prints, and what `feat skill show` prints can be diffed against an installed
...
// the path it would write, whether a file is already there to replace, and a
// refusal, as the same SkillDivergedError the install itself would return.
//
...
// InstallSkill writes the setup skill into dir, refusing to replace a file that
// is not what an install recorded writing (ADR-093).
//
// The record is written before the document, so no file exists that the record
// cannot name. An install interrupted between the two leaves a record ahead of
// the document, and the next install then refuses with the reason rather than
// guessing, while --force recovers it and loses only Feat's own earlier words.
...
func skillDiverged(current []byte, recordPath string) string {
	...
	if sum == skillChecksum(skillDocument) {
		// Already exactly what this binary writes, most likely from a record lost
		// or never written. Replacing it and refreshing the record loses nothing
		// anyone authored.
		return ""
	}
...
// writeSkillFile writes into a temporary file beside path and renames it in, so
// an interruption leaves the previous file rather than half of a new one.
